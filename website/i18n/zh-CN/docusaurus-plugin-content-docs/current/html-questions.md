...
- 开发建议：所有 HTML 代码（标签 / 属性名）一律小写，属性值按对应规范（如 UTF-8 大写、类名和 JS 保持一致）；  
- 面试考点：记住「HTML 标签 / 属性名不区分，CSS/JS 区分，属性值看场景」即可。  
- 「为什么 HTML 推荐小写？」，答出「统一风格 + 规避配套技术的大小写坑」
...
data-*是 HTML5 新增的自定义数据属性，允许你在 HTML 元素上存储与该元素相关的任意自定义数据，这些数据不会影响元素的显示，仅作为 “附加信息” 供 JavaScript 调用，是前端中 HTML 与 JS 之间传递数据的常用方式。  
核心规则：  
属性名必须以 data- 开头，后面可以跟任意字母、数字、连字符（-）、下划线（_）等（比如 data-id、data-user-name、data-product-price）。  
属性值可以是任意字符串（数字、文本、JSON 格式字符串等）。  
所有主流浏览器（Chrome、Firefox、Edge、Safari）都支持。  

**基础语法**  
在 HTML 中定义 data 属性：直接在元素标签上添加即可，如：`<div id="user" data-id="1001" data-user-name="张三" data-age="25">用户信息</div>`  
在 JavaScript 中操作 data 属性：通过dataset对象（推荐，更简洁），dataset是元素的内置对象，会自动将data-后的属性名转换为驼峰命名（比如data-user-name对应dataset.userName），直接读取 / 修改即可。
...
浏览器加载页面的核心步骤是：
`解析HTML → 构建DOM树 → 解析CSS → 构建CSSOM树 → 合并DOM+CSSOM生成渲染树 → 布局 → 绘制`
...
srcset 由「图片 URL + 描述符 + 逗号分隔」组成，有两种核心描述符：  
1. **密度描述符（x）：适配不同屏幕分辨率**  
   `<!-- 1x=普通屏，2x=Retina屏，3x=超高清屏 -->
    <img 
      src="image-300.jpg"  <!-- 降级兜底 -->
      srcset="image-300.jpg 1x, image-600.jpg 2x, image-900.jpg 3x"
      alt="示例图"
    >`
3. **宽度描述符（w）：适配不同视口大小（需配合 sizes）**  
   `<!-- w=图片实际宽度，sizes=浏览器计算「图片显示宽度」的规则 -->
    <img 
      src="image-300.jpg"
      srcset="image-300.jpg 300w, image-600.jpg 600w, image-900.jpg 900w"
      sizes="(max-width: 600px) 280px, 600px"  <!-- 视口≤600px时，图片显示280px；否则显示600px -->
      alt="示例图"
    >`
...
**总结**
1. srcset 的核心作用：让浏览器根据「设备 DPR + 视口大小」自动选择最优尺寸图片，兼顾性能和清晰度；
2. 处理流程（宽度描述符为例）：解析 sizes 算显示宽度 → 结合 DPR 算所需宽度 → 选≥所需宽度的最小图片 → 加载（失败则降级 src）；
3. 核心搭配：密度描述符（x）适配分辨率，宽度描述符（w）+ sizes 适配视口大小。
